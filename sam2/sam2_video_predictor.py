...
from sam2.utils.misc import concat_points, fill_holes_in_mask_scores, load_video_frames, _load_img_as_tensor
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def init_state_(
        ...
        if isinstance(template_images_dir, str) and os.path.isdir(template_images_dir):
            jpg_folder = template_images_dir
        ...
        images = torch.zeros(num_frames+1, 3, self.image_size, self.image_size, dtype=torch.float32)
        for n, img_path in enumerate(tqdm(img_paths, desc="frame loading")):
            images[n], _, _ = _load_img_as_tensor(img_path, self.image_size)
        images[-1], video_height, video_width = _load_img_as_tensor(test_image_path, self.image_size)
